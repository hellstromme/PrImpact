...
def _call_claude(client: anthropic.Anthropic, prompt: str, model: str = MODEL) -> str:
...
def call_api(client: anthropic.Anthropic, prompt: str, label: str, model: str = MODEL) -> dict:
    """Call Claude, log the raw response, and parse JSON. Returns {} on any failure."""
    try:
        raw = _call_claude(client, prompt, model)
