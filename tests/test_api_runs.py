...
def test_list_runs_returns_200(client):
    tc, (db, _) = client
    resp = tc.get("/api/runs", params={"repo": "/repo"})
...
def test_list_runs_filters_by_repo(client):
    tc, _ = client
    resp = tc.get("/api/runs", params={"repo": "/other-repo"})
    assert resp.json() == []


def test_list_runs_pagination(client):
    tc, _ = client
    page1 = tc.get("/api/runs", params={"repo": "/repo", "limit": 1, "offset": 0}).json()
    page2 = tc.get("/api/runs", params={"repo": "/repo", "limit": 1, "offset": 1}).json()
    assert len(page1) == 1
    assert len(page2) == 1
...
def test_get_run_404_for_unknown(client):
    tc, _ = client
    resp = tc.get("/api/runs/00000000-0000-0000-0000-000000000000")
...
def test_get_report_returns_200(client):
    tc, (db, ids) = client
    resp = tc.get(f"/api/runs/{ids[0]}/report")
    assert resp.status_code == 200


def test_get_report_contains_expected_fields(client):
    tc, (db, ids) = client
    data = tc.get(f"/api/runs/{ids[0]}/report").json()
...
def test_get_report_404_for_unknown(client):
    tc, _ = client
    resp = tc.get("/api/runs/00000000-0000-0000-0000-000000000000/report")
    assert resp.status_code == 404
    assert "error" in resp.json()["detail"]
